...
origpath="/home/betullam/Bilder/test/orig"
tifpath="/home/betullam/Bilder/test/renamed"
...
# Convert original images to tifs. If it is already a file with the file extension "tif", do NOT convert it.
# The reason is: we remove the original image after conversion. If the original AND the converted image have
# both the same filename (because both have the extension "tif"), both would be deleted.
for file in "$origpath/"*.{tiff,TIF,TIFF,jpg,jpeg,JPG,JPEG} ; do
...
# Loop over files that are sorted by file name with "ls -v". Only use image files with the given extensions.
#for file in `ls -v "$origpath/"*.{tif,tiff,TIF,TIFF,jpg,jpeg,JPG,JPEG}` ; do
for file in `ls -v "$origpath/"*.tif` ; do
	...
	# Rename the file
	mv $file "$origpath/$newFilename.tif"
done
